package mqtt_private

import (
	"encoding/json"
	"fmt"
	"net"
	"strings"

	"github.com/sirupsen/logrus"
	"github.com/spf13/viper"
)

var PrivateMqttConfig Config
var gatewayID string //通过初始化函数：GetGatewayID 在程序刚运行的时候获取

func MqttPrivateInit() error {
	// 初始化配置
	err := loadPrivateConfig()
	if err != nil {
		return err
	}
	createMqttClient()

	GetGatewayID() //和data_collect模块商定，网关ID通过网卡获取，那边也是通过getMACAddress 函数获取的地址来注册

	subscribe()
	return nil
}

// 连接到内网监控站的配置
func loadPrivateConfig() error {
	var configMap map[string]interface{}
	// 将 map 映射到 PrivateMqttConfig 结构体
	// 注意！！！yml文件中带_的key，是无法通过UnmarshalKey解析的
	err := viper.Unmarshal(&configMap)
	if err != nil {
		return fmt.Errorf("unable to decode into struct, %s", err)
	}
	// 将 map 转换为 json
	jsonStr, err := json.Marshal(configMap["private_mqtt"])
	if err != nil {
		return fmt.Errorf("unable to marshal config, %s", err)
	}

	// 将 json 转换为结构体
	err = json.Unmarshal(jsonStr, &PrivateMqttConfig)
	if err != nil {
		return fmt.Errorf("unable to unmarshal config, %s", err)
	}

	// 打印配置
	logrus.Debugf("mqtt_private config:%#v", PrivateMqttConfig)

	// 单独获取 broker 配置
	broker := viper.GetString("private_mqtt.broker")
	if broker == "" {
		broker = "localhost:1883"
		logrus.Println("Using default broker:", broker)
	}
	PrivateMqttConfig.Broker = broker

	// 单独获取 user 配置
	user := viper.GetString("private_mqtt.user")
	if user == "" {
		user = "root"
		logrus.Println("Using default user:", user)
	}
	PrivateMqttConfig.User = user

	// 单独获取 pass 配置
	pass := viper.GetString("private_mqtt.pass")
	if pass == "" {
		pass = "root"
		logrus.Println("Using default pass:", pass)
	}
	PrivateMqttConfig.Pass = pass

	// 单独获取 channel_buffer_size 配置
	channelBufferSize := viper.GetInt("private_mqtt.channel_buffer_size")
	if channelBufferSize == 0 {
		channelBufferSize = 10000
		logrus.Println("Using default channel_buffer_size:", channelBufferSize)
	}
	PrivateMqttConfig.ChannelBufferSize = channelBufferSize

	// 单独获取 write_workers 配置
	writeWorkers := viper.GetInt("private_mqtt.write_workers")
	if writeWorkers == 0 {
		writeWorkers = 10
		logrus.Println("Using default write_workers:", writeWorkers)
	}
	PrivateMqttConfig.WriteWorkers = writeWorkers

	// 单独获取 pool_size 配置
	poolSize := viper.GetInt("private_mqtt.telemetry.pool_size")
	if poolSize == 0 {
		poolSize = 100
		logrus.Println("Using default pool_size:", poolSize)
	}
	PrivateMqttConfig.Telemetry.PoolSize = poolSize

	// 单独获取 batch_size 配置
	batchSize := viper.GetInt("private_mqtt.telemetry.batch_size")
	if batchSize == 0 {
		batchSize = 100
		logrus.Println("Using default batch_size:", batchSize)
	}
	return nil
}
func GetGatewayID() string {
	if gatewayID != "" {
		return gatewayID
	}
	var err error
	gatewayID, err = getMACAddress("eth0")
	if err != nil {
		logrus.Errorf("GetGatewayID err:%v ", err)
		return ""
	}
	return gatewayID
}
func getMACAddress(interfaceName string) (string, error) {
	//return "1C:40:E8:11:69:54", nil
	iface, err := net.InterfaceByName(interfaceName)
	if err != nil {
		return "", err
	}
	macClean := strings.ReplaceAll(strings.ToUpper(iface.HardwareAddr.String()), ":", "")
	return macClean, nil
}
